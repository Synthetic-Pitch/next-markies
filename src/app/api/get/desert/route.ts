...
export async function GET() {
    ...
    try {
        const cachedKey = 'desert';
        
        // Get the total count of beverages in database
        const currentCount = await FoodsModel.countDocuments({ category: "desert" });
        ...
        const beverages = await FoodsModel.find({category: "desert"}).limit(50);
